...
from algo import (System, ExtendedKalmanFilter, vec_cholesky, design_ce_lqr, design_soc_lqr)

def run_simulation(sys, controller_type, K, dict_func, N_sim, x0_true, x0_hat, Sigma0):
    nx = sys.nx
    ekf = ExtendedKalmanFilter(sys)
    
    x_true_hist = np.zeros((nx, N_sim + 1))
    x_hat_hist = np.zeros((nx, N_sim + 1))
    tr_Sigma_hist = np.zeros(N_sim + 1)
    ...
    for k in range(N_sim):
        ...
        # 3. 真のシステムを更新
        w_k = np.random.multivariate_normal(np.zeros(nx), sys.Sigma_w)
        v_k = np.random.multivariate_normal(np.zeros(1), sys.Sigma_v)
        ...
        # 5. 履歴に保存
        cost = x_hat.T @ sys.Q @ x_hat + u_k.T @ sys.R @ u_k + np.trace(sys.Q @ Sigma)
        cost_hist.append(cost)
        x_true = x_true_next

    x_hat_hist[:, -1] = x_hat_post
    tr_Sigma_hist[-1] = np.trace(Sigma_post)

    est_error_sq = np.sum((x_hat_hist[:, :-1] - x_true_hist[:, :-1])**2, axis=0)
...
if __name__ == "__main__":
    ...
    # 3. シミュレーションの実行
    N_sim = 1000
    x0_true = np.random.multivariate_normal(np.zeros(sys.nx), np.eye(sys.nx))
    ...
    avg_err_ce = np.mean(ce_results["est_error_sq"])
    avg_err_soc = np.mean(soc_results["est_error_sq"])
    ...
    ax2.plot(np.sum(ce_results["x_hat"], axis=0)[:-1], 'red', label=r'CE-LQR ($\sum x_{k|k-1}^i$)')
    ax2.plot(np.sum(ce_results["x_true"], axis=0)[:-1], 'red', linestyle='--', alpha=0.7, label='CE-LQR (true state)')
    ax2.plot(np.sum(soc_results["x_hat"], axis=0)[:-1], 'black', label=r'SOC-LQR ($\sum x_{k|k-1}^i$)')
    ax2.plot(np.sum(soc_results["x_true"], axis=0)[:-1], 'black', linestyle='--', alpha=0.7, label='SOC-LQR (true state)')
